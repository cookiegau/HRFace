...
///
- (void)fetchDeviceInformationWithCompletion:(void (^ _Nonnull)(id <AWDeviceInformation> _Nullable, NSError * _Nullable))completion SWIFT_AVAILABILITY(ios,deprecated=19.7.0,message="Deprecated in favor of fetchApplicationAssignmentStatus");
/// Fetches app assignment status information from console.
/// On successful fetch, this method returns consoleVersion, isAppSupported, isDeviceManaged, enrollmentStatus via the completion handler.
/// On error, this method executes the completion handler with nil AppStatus related information and an error.
/// \param completion completion block to be passed in which executes on successful fetch / failure.
///
- (void)fetchApplicationAssignmentStatusWithCompletion:(void (^ _Nonnull)(id <AWApplicationStatusInformation> _Nullable, NSError * _Nullable))completion;
