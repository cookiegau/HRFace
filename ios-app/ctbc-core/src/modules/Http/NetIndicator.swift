...
open class NetIndicator: NSObject
{
	public static let shared = NetIndicator()
	...
	public static func addObserverQueue( _ queue: OperationQueue )
	{
		queue.addObserver( shared, forKeyPath: operationCountKey, options: .new, context: nil )
		shared.queues.append( queue )
	}

	public static func removeObserverQueue( _ queue: OperationQueue )
	{
	...
	{
		if keyPath != NetIndicator.operationCountKey { return }
		startIndicator()
	}
	...
	deinit
	{
		for q in queues { q.removeObserver( self, forKeyPath: NetIndicator.operationCountKey ) }
