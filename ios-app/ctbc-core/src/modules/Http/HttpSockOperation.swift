...
class HttpSockOperation: Operation, WebSocketDelegate
{
	...
	enum Sequence: Int
	{ case header, body, chunkedLength, chunkedBody, chunkBodyTail }
	...
	override func start()
	{
		...
		{
			let error = NSError( domain: Http.ErrorDomain, code: 1, userInfo: [ NSLocalizedDescriptionKey: "" ] )
	...
	func didRead( data: Data )
	{
		...
		switch sequence
		{
			...
			case .chunkedLength:
				...
				if scanner.scanHexInt64( &hexValue ) == false
				{
	...
	{
		let error = NSError( domain: Http.ErrorDomain, code: code, userInfo: [ NSLocalizedDescriptionKey: msg ] )
		compError( error )
	}
}
