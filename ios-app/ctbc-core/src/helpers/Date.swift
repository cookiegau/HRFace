...
/**
 ````
 case isoYear: => 1997
 case isoYearMonth: => 1997-07
 case isoDate: => 1997-07-16
 case isoDateTime: => 1997-07-16T19:20+01:00
 case isoDateTimeSec: => 1997-07-16T19:20:30+01:00
 case isoDateTimeMilliSec: => 1997-07-16T19:20:30.45+01:00
 case dotNet: => "/Date(1268123281843)/"
 case rss: => "Fri, 09 Sep 2011 15:26:08 +0200"
 case altRSS: => "09 Sep 2011 15:26:08 +0200"
 case httpHeader: => "Tue, 15 Nov 1994 12:45:26 GMT"
...
public enum DateFormatType
{
	...
	/// yyyy-MM-dd'T'HH:mmZ => 1997-07-16T19:20+01:00
	case isoDateTime

	/// yyyy-MM-dd'T'HH:mm:ssZ => 1997-07-16T19:20:30+01:00
	case isoDateTimeSec

	/// yyyy-MM-dd'T'HH:mm:ss.SSSZ => 1997-07-16T19:20:30.45+01:00
	case isoDateTimeMs

	/// "/Date(%d%d)/" => "/Date(1268123281843)/"
	case dotNet

	/// "EEE, d MMM yyyy HH:mm:ss ZZZ" => "Fri, 09 Sep 2011 15:26:08 +0200"
	case rss

	/// "d MMM yyyy HH:mm:ss ZZZ" => "09 Sep 2011 15:26:08 +0200"
	case altRSS

	/// "EEE, dd MM yyyy HH:mm:ss ZZZ" => "Tue, 15 Nov 1994 12:45:26 GMT"
	case httpHeader
	...
	var stringFormat: String
	{
		switch self
		{
			case .log: return "yyyyMMddHHmmss.SSS"
			case .full: return "yyyy-MM-dd HH:mm:ss"
			case .fullMs: return "yyyy-MM-dd HH:mm:ss.SSS"
			case .isoYear: return "yyyy"
			case .isoYearMonth: return "yyyy-MM"
			case .isoDate: return "yyyy-MM-dd"
			case .isoDateTime: return "yyyy-MM-dd'T'HH:mmZ"
			case .isoDateTimeSec: return "yyyy-MM-dd'T'HH:mm:ssZ"
			case .isoDateTimeMs: return "yyyy-MM-dd'T'HH:mm:ss.SSSZ"
...
public extension Date
{
	...
	*/
	init?( fromString string: String, format: DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = Foundation.Locale.current, isLenient: Bool = true )
	{
		...
		let formatter = Date.cachedDateFormatters.cachedFormatter( format.stringFormat, timeZone: timeZone.timeZone, locale: locale, isLenient: isLenient )
		guard let date = formatter.date( from: string ) else { return nil }
		self.init( timeInterval: 0, since: date )
	}

	func toStringBy( style: DateStyleType = .short ) -> String
	{
		switch style
		{
			case .short:
				return self.toStringBy( dateStyle: .short, timeStyle: .short, isRelative: false )
			case .medium:
				return self.toStringBy( dateStyle: .medium, timeStyle: .medium, isRelative: false )
			case .long:
				return self.toStringBy( dateStyle: .long, timeStyle: .long, isRelative: false )
			case .full:
				return self.toStringBy( dateStyle: .full, timeStyle: .full, isRelative: false )
			case .ordinalDay:
				...
				return formatter.string( from: self.componentBy( .day )! as NSNumber )!
			case .weekday:
				let weekdaySymbols = Date.cachedDateFormatters.cachedFormatter().weekdaySymbols!
				let string = weekdaySymbols[componentBy( .weekday )! - 1] as String
				return string
			case .shortWeekday:
				let shortWeekdaySymbols = Date.cachedDateFormatters.cachedFormatter().shortWeekdaySymbols!
				return shortWeekdaySymbols[componentBy( .weekday )! - 1] as String
			case .veryShortWeekday:
				let veryShortWeekdaySymbols = Date.cachedDateFormatters.cachedFormatter().veryShortWeekdaySymbols!
				return veryShortWeekdaySymbols[componentBy( .weekday )! - 1] as String
			case .month:
				let monthSymbols = Date.cachedDateFormatters.cachedFormatter().monthSymbols!
				return monthSymbols[componentBy( .month )! - 1] as String
			case .shortMonth:
				let shortMonthSymbols = Date.cachedDateFormatters.cachedFormatter().shortMonthSymbols!
				return shortMonthSymbols[componentBy( .month )! - 1] as String
			case .veryShortMonth:
				let veryShortMonthSymbols = Date.cachedDateFormatters.cachedFormatter().veryShortMonthSymbols!
				return veryShortMonthSymbols[componentBy( .month )! - 1] as String
		}
	}

	func toStringBy( format: DateFormatType, timeZone: TimeZoneType = .local, locale: Locale = Locale.current ) -> String
	{
		switch format
		{
			case .dotNet:
	...
	func toStringBy( dateStyle: DateFormatter.Style, timeStyle: DateFormatter.Style, isRelative: Bool = false, timeZone: Foundation.TimeZone = Foundation.NSTimeZone.local, locale: Locale = Locale.current ) -> String
	{
	...
	func toStringWithRelativeTime( strings: [RelativeTimeStringType: String]? = nil ) -> String
	{
		...
		if d < 28
		{
			if isPast
			{
				...
				else
				{
					...
					return String( format: string, Double( abs( sinceBy( Date(), in: .week ) ) ) )
				}
			}
			else
			{
				if compareBy( .isNextWeek )
				{
		...
		if compareBy( .isThisYear )
		{
			if isPast
			{
				if compareBy( .isLastMonth )
				{
				...
				else
				{
					...
					return String( format: string, Double( abs( sinceBy( Date(), in: .month ) ) ) )
				}
			}
			else
			{
				if compareBy( .isNextMonth )
				{
		...
		if isPast
		{
			...
			else
			{
				...
				return String( format: string, Double( abs( sinceBy( Date(), in: .year ) ) ) )
			}
		}
		else
		{
			if compareBy( .isNextYear )
			{
	...
	func componentBy( _ component: DateComponentType ) -> Int?
	{
	...
	func compareBy( _ comparison: DateComparisonType ) -> Bool
	{
		switch comparison
		{
			case .isToday:
				return compareBy( .isSameDay( as: Date() ) )
			case .isTomorrow:
				let comparison = Date().adjustBy( .day, offset: 1 )
				return compareBy( .isSameDay( as: comparison ) )
			case .isYesterday:
				let comparison = Date().adjustBy( .day, offset: -1 )
				return compareBy( .isSameDay( as: comparison ) )
			case .isSameDay( let date ):
				return componentBy( .year ) == date.componentBy( .year ) && componentBy( .month ) == date.componentBy( .month ) && componentBy( .day ) == date.componentBy( .day )
			case .isThisWeek:
				return self.compareBy( .isSameWeek( as: Date() ) )
			case .isNextWeek:
				let comparison = Date().adjustBy( .week, offset: 1 )
				return compareBy( .isSameWeek( as: comparison ) )
			case .isLastWeek:
				let comparison = Date().adjustBy( .week, offset: -1 )
				return compareBy( .isSameWeek( as: comparison ) )
			case .isSameWeek( let date ):
				if componentBy( .week ) != date.componentBy( .week ) { return false }
				// Ensure time interval is under 1 week
				return abs( self.timeIntervalSince( date ) ) < Date.weekInSeconds
			case .isThisMonth:
				return self.compareBy( .isSameMonth( as: Date() ) )
			case .isNextMonth:
				let comparison = Date().adjustBy( .month, offset: 1 )
				return compareBy( .isSameMonth( as: comparison ) )
			case .isLastMonth:
				let comparison = Date().adjustBy( .month, offset: -1 )
				return compareBy( .isSameMonth( as: comparison ) )
			case .isSameMonth( let date ):
				return componentBy( .year ) == date.componentBy( .year ) && componentBy( .month ) == date.componentBy( .month )
			case .isThisYear:
				return self.compareBy( .isSameYear( as: Date() ) )
			case .isNextYear:
				let comparison = Date().adjustBy( .year, offset: 1 )
				return compareBy( .isSameYear( as: comparison ) )
			case .isLastYear:
				let comparison = Date().adjustBy( .year, offset: -1 )
				return compareBy( .isSameYear( as: comparison ) )
			case .isSameYear( let date ):
				return componentBy( .year ) == date.componentBy( .year )
			case .isInTheFuture:
				return self.compareBy( .isLater( than: Date() ) )
			case .isInThePast:
				return self.compareBy( .isEarlier( than: Date() ) )
			...
			case .isWeekday:
				return !compareBy( .isWeekend )
			case .isWeekend:
				let range = Calendar.current.maximumRange( of: Calendar.Component.weekday )!
				return ( componentBy( .weekday ) == range.lowerBound || componentBy( .weekday ) == range.upperBound - range.lowerBound )
		}
	}

	func adjustBy( _ component: DateComponentType, offset: Int ) -> Date
	{
	...
	func adjustBy( hour: Int?, minute: Int?, second: Int?, day: Int? = nil, month: Int? = nil ) -> Date
	{
	...
	func dateFor( _ type: DateForType, calendar: Calendar = Calendar.current ) -> Date
	{
		...
		{
			case .startOfDay:
				return adjustBy( hour: 0, minute: 0, second: 0 )
			case .endOfDay:
				return adjustBy( hour: 23, minute: 59, second: 59 )
			case .startOfWeek:
				return calendar.date( from: calendar.dateComponents( [ .yearForWeekOfYear, .weekOfYear ], from: self ) )!
			case .endOfWeek:
				let offset = 7 - componentBy( .weekday )!
				return adjustBy( .day, offset: offset )
			case .startOfMonth:
				return adjustBy( hour: 0, minute: 0, second: 0, day: 1 )
			case .endOfMonth:
				let month = ( componentBy( .month ) ?? 0 ) + 1
				return adjustBy( hour: 0, minute: 0, second: 0, day: 0, month: month )
			case .tomorrow:
				return adjustBy( .day, offset: 1 )
			case .yesterday:
				return adjustBy( .day, offset: -1 )
			case .nearestMinute( let nearest ):
				let minutes = ( componentBy( .minute )! + nearest / 2 ) / nearest * nearest
				return adjustBy( hour: nil, minute: minutes, second: nil )
			case .nearestHour( let nearest ):
				let hours = ( componentBy( .hour )! + nearest / 2 ) / nearest * nearest
				return adjustBy( hour: hours, minute: 0, second: nil )
		}
	}

	func sinceBy( _ date: Date, in component: DateComponentType ) -> Int64
	{
		switch component
		{
			case .second:
	...
	{
		let distanceToStartOfWeek = Date.dayInSeconds * Double( self.componentBy( .weekday )! - 1 )
		let interval: TimeInterval = self.timeIntervalSinceReferenceDate - distanceToStartOfWeek
		return Date( timeIntervalSinceReferenceDate: interval ).componentBy( .day )!
	}
	...
	{
		private static let cachedDateFormattersQueue = DispatchQueue( label: "date-formatter-queue", attributes: .concurrent )

		private static let cachedNumberFormatterQueue = DispatchQueue( label: "number-formatter-queue", attributes: .concurrent )
		...
		{
			let dateFormatter = concurrentFormatterCache.cachedDateFormattersQueue.sync
			{ () -> DateFormatter? in
		...
		{
			let numberFormatter = concurrentFormatterCache.cachedNumberFormatterQueue.sync
			{ () -> NumberFormatter in
		...
		public func cachedFormatter( _ format: String = DateFormatType.standard.stringFormat, timeZone: Foundation.TimeZone = Foundation.TimeZone.current, locale: Locale = Locale.current, isLenient: Bool = true ) -> DateFormatter
		{
...
public extension Date
{
	static func parseBy( _ dateString: String, _ format: String = "yyyy-MM-dd HH:mm:ss" ) -> Date?
	{
		let formatter = DateFormatter()
		formatter.dateFormat = format
...
public extension TimeInterval
{
	...
	{
		get { return Int( ( self.truncatingRemainder( dividingBy: 1 ) ) * 1000 ) }
	}
}
